...
def predict(target_x, target_y):
    ...
    preds = model(Y_norm)
    preds = torch.max(preds,0.05*torch.ones(preds.shape))
    return preds / torch.sum(preds)

proba_unary = predict(target_x, target_y)
...
def main():
    ...
    print(f"Target function: {target_formula}")
...
print(f"Model prediction :{proba_unary}")
